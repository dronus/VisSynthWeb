...
function Lcd(config) {
  ...
  this.rs = new Gpio(config.rs, 'high'); // reg. select, output, initially low
  this.e = new Gpio(config.e, 'high'); // enable, output, initially low

  this.data = []; // data bus, db4 thru db7, outputs, initially low
  for (i = 0; i < config.data.length; i += 1) {
    this.data.push(new Gpio(config.data[i], 'high'));
  }
